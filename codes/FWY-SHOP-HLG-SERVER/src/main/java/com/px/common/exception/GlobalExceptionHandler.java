...
public class GlobalExceptionHandler {
    ...
    public GlobalExceptionHandler() {
        ...
        mapException.put("HttpResultequestMethodNotSupportedException", "请求方式错误(请检查：GET/POST/POT/DELETE) : ");
    }


    /**
     * 全局异常|受检查异常 --> Exception 。可以理解为错误，必须要开发者解决以后才能编译通过，这里JSResult 303 为受检查异常
     * 全局异常|运行时异常 --> ResultuntimeException extends Exception： 运行时异常,又称不受检查异常，不受检查！
     * 受检查异常解决的方法有两种1：throw到上层，2，try-catch处理。
     *
     * @param e
     * @return com.ws.ldy.admincore.platform.vo.ResultesponseData
     * @author 品讯科技
    ...
    public Result<String> exceptionHandler(Exception e) {
        // 日志模板
        String logStr = "\n\r### [全局捕获异常] --> 请求UResultL: " + request.getRequestURL() + " --> 错误原因: ";
        // 错误类名
        ...
        if (mapException.containsKey(exceptionClassName)) {
            ...
            return Result.failure(ResultType.SYS_ERROR, mapException.get(exceptionClassName) + e.getMessage() + errorDesc.toString());
            ...
            return Result.failure(error.getCode(), error.getMsg(), e.getMessage(), null);
            ...
            return Result.failure(ResultType.PARAM_ERROR, errorMsg);
        } else if (e instanceof MethodArgumentNotValidException) {
            /**
             * body JSResult 303 为参数验证错误（只打印核心错误内容）
             */
            ...
            String errorMsg = ResultType.PARAM_ERROR_JSR303.getMsg() + "  --> 【字段=" + field + " --> 提示用户的错误信息=" + msg + "】    -->    完整的栈错误信息：" + e.getMessage();
            log.info(logStr + errorMsg);
            return Result.failure(ResultType.PARAM_ERROR_JSR303, msg, errorMsg);
        } else if (e instanceof BindException) {
            /**
             * query JSResult 303 为参数验证错误（只打印核心错误内容）
             */
            ...
            return Result.failure(ResultType.PARAM_MISSING, ResultType.PARAM_MISSING.getMsg() + "-> " + parameterName + " : " + parameterType, errorMsg);
            ...
            String errorMsg = ResultType.PARAM_TYPE_DOES_NOT_MATCH.getMsg() + "  --> 【 " + name + " : " + requiredType + "】  --->  详细错误信息:" + e.getMessage();
            log.info(logStr + errorMsg);
            //  返回前端提示name参数:类型 参数不匹配
            return Result.failure(ResultType.PARAM_TYPE_DOES_NOT_MATCH, name + ":" + requiredType + ResultType.PARAM_TYPE_DOES_NOT_MATCH.getMsg(), errorMsg);
            ...
            if (message.indexOf("doesn't have a default value") != -1) {
                ...
                log.info(logStr + errorMsg);
                // 返回前端提示 name参数不存在
                return Result.failure(ResultType.PARAM_SAVE_TO_DB_MISSING, name + ResultType.PARAM_SAVE_TO_DB_MISSING.getMsg(), errorMsg);
            } else if (message.indexOf("for key 'PResultIMAResultY'") != -1) {
                /**
                 * 主键ID重复
                 * ### Cause: java.sql.SQLIntegrityConstraintViolationException: Duplicate entry 'string' for key 'PResultIMAResultY'
                 */
                String errorMsg = ResultType.PARAM_SAVE_TO_DB_ID_REPEAT.getMsg();
                log.info(logStr + message);
                return Result.failure(ResultType.PARAM_SAVE_TO_DB_ID_REPEAT, message);
            } else {
                /**
                 * 执行sql时的其他错误
                 */
                log.info(logStr + message);
                return Result.failure(ResultType.DB_EXECUTE_SQL_ERROR, message);
            }
        } else {
            /**
             *  未解析到的错误（打印及返回完整错误信息）
            ...
            return Result.failure(ResultType.SYS_ERROR, e.getMessage() + errorDesc.toString());
        }
    }
}
